<template>
  <div class="flex-1 divide-y-2 p-4 text-lg text-highlight-text">
    ...
    <div class="flex text-sm" v-for="(probValue, key) in profilConfig" :class="key % 2 ? 'bg-slate-300/20' : 'bg-secondary-content'">
      <div class="w-1/2 p-2">{{ probValue.label }}</div>
      <div class="w-1/2 p-2">{{ user[probValue.propKey] }}</div>
    </div>
    <div class="flex text-sm " :class="key % 2 ? 'bg-slate-300/20' : 'bg-secondary-content'">
      <div class="w-1/2 p-2">Gehalt</div>
      <div class="w-1/2 p-2">
        <BaseEditInput v-model="revenue.value" @leave="updateRevenue()" :input-extension="'€'" />
      </div>
    </div>
  </div>
</template>
...
onMounted(async () => {
  loading.value = true;
  const revenues = (await revenueClient.getRevenuesEndpoint())
    .revenues;
...
const updateRevenue = async () => {
  const request: UpdateRevenueRequest = { uRevenueDto: { id: revenue.value.id, value: revenue.value.value } }
