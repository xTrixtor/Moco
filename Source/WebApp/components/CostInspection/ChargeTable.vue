<template>
  <div v-if="selectedMontlyBudget" class="h-full">
    <div class="flex-center items-center underlineAnimation mb-2">
      ...
      <Icon class="flex justify-center items-center cursor-pointer text-green-600 duration-300 mr-1 opacity-60 hover:opacity-100" size="1.5rem" name="ant-design:edit-outlined" @click="() => (editModalVis = true)"/>
    </div>
  ...
  <Dialog v-model:visible="editModalVis" modal :header="'Bearbeitung von: ' + selectedMontlyBudget.name" :style="{ width: '25rem' }">
...
const editMonthlyBudget : Ref<MonthlyBudgetUDto> = ref({name:selectedMontlyBudget.value.name, id:selectedMontlyBudget.value.id, limit:selectedMontlyBudget.value.limit})
...
const handleUpdateMonthlyBudget = async () => {
  ...
  const response = await useApiStore().InspectionClient.updateMonthlyBudgetEnpoint({limit: limit, name: name, monthlyBudgetId: editMonthlyBudget.value.id});
