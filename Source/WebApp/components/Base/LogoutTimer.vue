<template>
  ...
  >
    <div class="divide-x-2 flex h-full items-center">
      ...
      <p class="pl-2 text-white w-12 flex justify-center items-center">
        <Icon
  ...
  >
    <div v-if="text" class="divide-x-2 flex h-full items-center">
      ...
      <p class="pl-2 text-white w-12">{{ text }}</p>
    </div>
    <div
      v-else
      class="w-full h-full flex justify-center items-center bg-gray/25"
    >
      <div role="status">
        <svg
          aria-hidden="true"
...
const handleRefreshClick = async () => {
  ...
  await useUserStore().refreshAuthTokenAsync(refreshToken);
