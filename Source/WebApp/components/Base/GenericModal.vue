<template>
  <div class="grid place-items-center absolute top-0 left-0 h-screen w-full">
    <div class="flex flex-col justify-between p-4 border-2 rounded-lg m-2 bg-foreground w-1/2 min-h-[50vh]">
      <div @click="handleClose" class="h-full w-full flex-center justify-between py-2">
        <p> {{ props.title }}</p>
        <svg class="fill-primary text-primary cursor-pointer" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="white" d="M6.4 19L5 17.6l5.6-5.6L5 6.4L6.4 5l5.6 5.6L17.6 5L19 6.4L13.4 12l5.6 5.6l-1.4 1.4l-5.6-5.6z"/></svg>
      </div>
      ...
      >
        ...
        <component
          ...
          @inputChange="(propName:string, newVal:any) => editObject[propName] = newVal"
        />
      </div>
      </div>
      <div class="flex w-full justify-end mt-4">
        <Button
          v-if="props.actionBtnTxt"
          @click="props.actionCall"
          >{{ props.actionBtnTxt }}</Button
        >
      </div>
    </div>
  </div>
